...
export default function Finder({ openApp }: { openApp?: (id: string) => void }) {
  ...
  const getFiles = () => {
    ...
    return allFiles.filter((f) => f.parentId === currentPath);
  };
    ...
    <div className="flex flex-col h-full text-white select-none bg-[#1e1e1f] rounded-lg overflow-hidden font-sans border border-white/5 shadow-2xl">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-48 bg-[#1e1e1f]/40 backdrop-blur-2xl border-r border-black/30 p-2.5 flex flex-col space-y-0.5 shadow-[inset_-1px_0_0_0_rgba(255,255,255,0.05)]">
          <div className="text-[11px] font-bold opacity-30 px-3 py-2 uppercase tracking-tight">
            Favorites
          </div>
          {favorites.map((item) => (
            <div
            ...
            >
              <span
                className={`${item.icon} text-[19px] ${currentPath === item.name ? "text-blue-400" : "text-blue-500/80 group-hover:text-blue-400"}`}
              />
              <span className={currentPath === item.name ? "font-medium" : ""}>
                {item.name}
              </span>
            </div>
          ))}
        ...
        {/* Main Content */}
        <div
          className="flex-1 overflow-y-auto bg-[#1a1a1b] shadow-inner"
        ...
        >
          {viewingFile ? (
            ...
            <div className="p-6 grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-x-6 gap-y-8 content-start">
              {currentFiles.map((item) => (
                <div
                  ...
                  onDoubleClick={(e) => {
                    ...
                    else if (item.type === "file" && item.content) setViewingFile(item);
                  }}
                  className="flex flex-col items-center group cursor-default"
                >
                  <div
                    className={`size-20 flex items-center justify-center mb-2 rounded-xl transition-all duration-200 relative ${
                      selectedId === item.id
                  ...
                  >
                    {item.type === "app" ? (
                      ...
                      <span
                        className={`${item.icon} text-[72px] ${item.color} drop-shadow-xl`}
                      />
                    )}
                  </div>
                  <span
                    className={`text-[12px] text-center px-2 py-0.5 rounded-sm transition-colors break-all leading-tight max-w-full font-medium ${
                      selectedId === item.id
